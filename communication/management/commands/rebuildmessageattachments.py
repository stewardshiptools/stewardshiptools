...
from communication.models import HarvestMessageAttachment, HarvestMessage
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        attachments_without_messages = HarvestMessageAttachment.objects.filter(
            message=None
        ).order_by(
            'id'
        )
        ...
        for message_record in HarvestMessage.objects.all().order_by('id'):
            ...
            if message.is_multipart():
                for part in message.walk():
                    ...
                    if digest in ATTACHMENT_HASH_MAP:
                        attachment = HarvestMessageAttachment.objects.get(
