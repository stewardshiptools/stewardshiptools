...
function build_popup(feature) {
    ...
    if (feature.properties.distance) {
        popup.innerHTML += '<br /><strong>Distance: ' + feature.properties.distance.value + ' ' + feature.properties.distance.unit + '</strong>';
    }
    ...
    for (var i = 0; i < spatialreport_items.length; i++) {
        ...
        for (var shape_type in item.urls) {
            ...
            $.ajax({
                ...
                done: function done(url, options) {
                    map.spin(false);
                }
            });
        }
...
ReactDOM.render(React.createElement(LeafletMap, { width: 'auto', height: '500px', settings: map_settings, map_id: spatialreport_feature_map_attach_id, callback: get_data_build_map }), document.getElementById(spatialreport_feature_map_attach_id));
